...
| Páginas de confirmación | `CURRENT_IMPLEMENTED_TRUTH` | Tres destinos `noindex` usan guards de sesión contra recarga y emiten eventos de presupuesto, WhatsApp o videollamada. | [gracias-videollamada.html](../../gracias-videollamada.html) emite dos acciones Ads en la primera carga; fuente única, clasificación y deduplicación están `PENDING_TO_VALIDATE` en [MARKETING_ANALYTICS_CONTRACT.md](MARKETING_ANALYTICS_CONTRACT.md). |
| Medios y movimiento | `CURRENT_IMPLEMENTED_TRUTH` | Imágenes, video hero, carrusel, reveal y elementos ambientales en [media](../../media), CSS y runtimes. | Calidad visual, performance y accesibilidad requieren sus gates; procedencia y derechos se registran en [MEDIA_PROVENANCE.md](MEDIA_PROVENANCE.md). |
